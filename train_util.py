...
def prune_train_iter(net, epochs=100, batch_size=128, lr=0.01, reg=5e-4):
    """
    Training a network
    :param net:
    :param epochs:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            print("Saving...")
            torch.save(net.state_dict(), "net_prune_train_iter.pt")
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for n, m in net.named_modules():
                if isinstance(m, PruneLinear):
                    m.linear.weight.grad.data *= torch.from_numpy(m.mask).float().cuda()
                elif isinstance(m,PrunedConv):
                   m.conv.weight.grad.data *= torch.from_numpy(m.mask).float().cuda()
            """
            ---------------------------------------------------------
            """
